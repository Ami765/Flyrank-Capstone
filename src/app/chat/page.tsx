...
export default function StreamingChatPage() {
    ...
    const input = (chatInstance as any).input;
    const handleInputChange = (chatInstance as any).handleInputChange || (chatInstance as any).onInputChange;
    const handleSubmit = (chatInstance as any).handleSubmit;
        ...
        <div className="max-w-4xl mx-auto flex flex-col h-[80vh] border border-zinc-200 rounded-2xl bg-white overflow-hidden shadow-sm mt-4 relative">
            ...
            >
                {messages && messages.map((m: any) => (
                    ...
                    >
                        <div className={`h-8 w-8 rounded-lg flex items-center justify-center border shrink-0 ${m.role === "user" ? "bg-zinc-900 border-zinc-800 text-white" : "bg-sky-50 border-sky-200 text-sky-600"
                            }`}>
                            {m.role === "user" ? <User size={16} /> : <Bot size={16} />}
                        </div>
                        <div className={`p-4 rounded-xl border text-sm shadow-sm leading-relaxed whitespace-pre-wrap ${m.role === "user" ? "bg-zinc-900 border-zinc-800 text-white" : "bg-white border-zinc-200 text-zinc-800"
                            }`}>
                            {m.content}
                        </div>
                    </div>
